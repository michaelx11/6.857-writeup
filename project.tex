...
The inspiration for this project was a description of a security flaw in Snapchat, published on the internet.\cite{snapchatFlaw1} The idea behind the Snapchat application is that pictures sent to and from users will disappear within ten seconds of being opened. However, rather than being erased from the device, images are actually stored permanently in device memory unless manually deleted. Snapchat only prevents casual users from seeing these stored images through limitations of its user interface. Users should also be restricted from viewing these images directly becasue they are encrypted. Unfortunately, it encrypts them using the encryption key "M02cnQ51Ji97vwT4," which has been hard-coded into the application code. Once this key is found and known, you can use it to decrypt these images, accessing images you're not meant to be able to see. We were easily able to find this key ourselves in the source code and the idea for our project was born.
